...
public class DaoCobra {
	...
	public boolean alterarCobra(Cobra cobra) {
		...
		String comandoSqlInsert = "UPDATE tb_cobra SET nome = ?, tipoveneno = ?, preco = ?,cep = ?,localidade = ?,logradouro = ?, bairro =?, uf = ? WHERE caf = ? "; // Base do comando
																									// SQL
		...
		try {
			...
			preparaOcomandoSQL.setString(3, cobra.getPreco().toString());
			preparaOcomandoSQL.setString(4, cobra.getEndereco().getCep());
			preparaOcomandoSQL.setString(5, cobra.getEndereco().getLocalidade());
			preparaOcomandoSQL.setString(6, cobra.getEndereco().getLogradouro());
			preparaOcomandoSQL.setString(7, cobra.getEndereco().getBairro());
			preparaOcomandoSQL.setString(8, cobra.getEndereco().getUf());
			preparaOcomandoSQL.setString(9, cobra.getCAF());// Coloca o valor no campo cpf
										//o numeral é conforme os parametros em interrogação ?
			preparaOcomandoSQL.execute(); // Executa o comando no banco de dados
